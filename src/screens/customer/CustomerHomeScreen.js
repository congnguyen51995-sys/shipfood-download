import React, { useState } from 'react';
import {
  View, Text, StyleSheet, FlatList, TouchableOpacity,
  TextInput, Image, ScrollView, StatusBar, Modal,
} from 'react-native';
import { useApp } from '../../context/AppContext';
import { useAuth } from '../../context/AuthContext';
import { formatCurrency, COLORS, CATEGORIES } from '../../utils/format';
import { Ionicons } from '@expo/vector-icons';

const ToppingModal = ({ visible, item, shopAlternatives = [], onClose, onConfirm }) => {
  const [selected, setSelected] = useState([]);
  const [activeItem, setActiveItem] = useState(null);

  React.useEffect(() => {
    if (item) { setActiveItem(item); setSelected([]); }
  }, [item]);

  if (!activeItem) return null;

  const toggle = (t) =>
    setSelected(prev => prev.find(x => x.id === t.id) ? prev.filter(x => x.id !== t.id) : [...prev, t]);

  const toppingTotal = selected.reduce((s, t) => s + t.price, 0);

  // Tất cả nguồn: nhà hàng (item gốc, không có shopId) + shop alternatives
  const restaurantItem = item?.shopId ? null : item;
  const allSources = [
    ...(restaurantItem ? [{ ...restaurantItem, _sourceLabel: 'Nhà hàng' }] : []),
    ...shopAlternatives.map(s => ({ ...s, _sourceLabel: s.shopName })),
  ];
  const showSourcePicker = allSources.length > 1 || (item?.shopId && allSources.length >= 1);

  return (
    <Modal visible={visible} animationType="fade" transparent onRequestClose={onClose}>
      <View style={styles.overlay}>
        <View style={styles.sheet}>
          <View style={styles.sheetHeader}>
            <View style={styles.handle} />
            <TouchableOpacity style={styles.closeBtn} onPress={onClose} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
              <Ionicons name="close" size={20} color={COLORS.gray} />
            </TouchableOpacity>
          </View>

          {/* Chọn nguồn */}
          {showSourcePicker && (
            <View style={styles.sourceSection}>
              <Text style={styles.sourceLabel}>Chọn nơi đặt:</Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.sourceRow}>
                {allSources.map((src, i) => {
                  const active = activeItem.id === src.id;
                  return (
                    <TouchableOpacity key={src.id}
                      style={[styles.sourceChip, active && styles.sourceChipActive]}
                      onPress={() => { setActiveItem(src); setSelected([]); }}>
                      <Ionicons
                        name={src.shopId ? 'storefront-outline' : 'restaurant-outline'}
                        size={12} color={active ? '#fff' : COLORS.gray} />
                      <Text style={[styles.sourceChipText, active && styles.sourceChipTextActive]}>
                        {src._sourceLabel}
                      </Text>
                      <Text style={[styles.sourceChipPrice, active && { color: 'rgba(255,255,255,0.85)' }]}>
                        {formatCurrency(src.price)}
                      </Text>
                    </TouchableOpacity>
                  );
                })}
              </ScrollView>
            </View>
          )}

          <View style={styles.itemRow}>
            <View style={styles.itemIcon}>
              {activeItem.image
                ? <Image source={{ uri: activeItem.image }} style={styles.itemImg} />
                : <Ionicons name="fast-food-outline" size={30} color={COLORS.primary} />}
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.itemName}>{activeItem.name}</Text>
              {activeItem.shopId && (
                <View style={styles.shopSourceTag}>
                  <Ionicons name="storefront-outline" size={11} color="#2196F3" />
                  <Text style={styles.shopSourceText}>{activeItem.shopName}</Text>
                </View>
              )}
              <Text style={styles.itemPrice}>{formatCurrency(activeItem.price)}</Text>
            </View>
          </View>

          {activeItem.toppings?.length > 0 && (
            <>
              <Text style={styles.toppingTitle}>Chọn món kèm (tuỳ chọn)</Text>
              <ScrollView style={{ maxHeight: 200 }}>
                {activeItem.toppings.map(t => {
                  const active = !!selected.find(x => x.id === t.id);
                  return (
                    <TouchableOpacity key={t.id} style={[styles.tRow, active && styles.tRowActive]} onPress={() => toggle(t)}>
                      <View style={[styles.check, active && styles.checkActive]}>
                        {active && <Ionicons name="checkmark" size={13} color="#fff" />}
                      </View>
                      <Text style={[styles.tName, active && { color: COLORS.primary, fontWeight: '600' }]}>{t.name}</Text>
                      <Text style={[styles.tPrice, active && { color: COLORS.primary }]}>+{formatCurrency(t.price)}</Text>
                    </TouchableOpacity>
                  );
                })}
              </ScrollView>
              <View style={styles.totalRow}>
                <Text style={styles.totalLabel}>Tổng:</Text>
                <Text style={styles.totalVal}>{formatCurrency(activeItem.price + toppingTotal)}</Text>
              </View>
            </>
          )}
          <View style={styles.modalBtns}>
            <TouchableOpacity style={styles.skipBtn} onPress={() => { setSelected([]); onClose(); }}>
              <Text style={styles.skipText}>Hủy</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.addBtn} onPress={() => { onConfirm(activeItem, selected); setSelected([]); onClose(); }}>
              <Ionicons name="cart-outline" size={17} color="#fff" />
              <Text style={styles.addText}>Thêm vào giỏ</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
    </Modal>
  );
};

const isInSaleWindow = (item) => {
  if (!item.saleStart && !item.saleEnd) return true;
  const now = new Date();
  const nowMins = now.getHours() * 60 + now.getMinutes();
  const parse = (t) => { const [h, m] = t.split(':').map(Number); return h * 60 + (m || 0); };
  const start = item.saleStart ? parse(item.saleStart) : 0;
  const end = item.saleEnd ? parse(item.saleEnd) : 1440;
  return start <= end ? (nowMins >= start && nowMins < end) : (nowMins >= start || nowMins < end);
};

export default function CustomerHomeScreen({ navigation, route }) {
  const { menuItems, addToCart, getCartCount, restaurantInfo } = useApp();
  const { currentUser } = useAuth();
  const [search, setSearch] = useState('');
  const [cat, setCat] = useState(route?.params?.filterCat || 'Tất cả');
  const [modal, setModal] = useState({ visible: false, item: null });

  // Nhận filterCat từ trang chủ
  React.useEffect(() => {
    if (route?.params?.filterCat) setCat(route.params.filterCat);
    if (route?.params?.openItem) setModal({ visible: true, item: route.params.openItem });
  }, [route?.params]);
  const [now, setNow] = useState(new Date());

  // Cập nhật giờ mỗi phút để kiểm tra giờ mở bán
  React.useEffect(() => {
    const t = setInterval(() => setNow(new Date()), 60000);
    return () => clearInterval(t);
  }, []);

  const isGuest = !currentUser;
  const cartCount = getCartCount(currentUser?.id || '');

  // Tách món nhà hàng và món shop
  const restaurantItems = menuItems.filter(i => !i.shopId);
  const shopItemsAll = menuItems.filter(i => !!i.shopId);

  // Món shop không có tên trùng với nhà hàng → hiện riêng trong list
  const shopNamesMatchingRestaurant = new Set(restaurantItems.map(i => i.name.toLowerCase()));
  const shopOnlyItems = shopItemsAll.filter(i => !shopNamesMatchingRestaurant.has(i.name.toLowerCase()));

  // Danh sách hiển thị = nhà hàng + shop-only
  const allDisplayItems = [...restaurantItems, ...shopOnlyItems];

  const filtered = allDisplayItems.filter(i =>
    i.name.toLowerCase().includes(search.toLowerCase()) &&
    (cat === 'Tất cả' || i.category === cat)
  );

  // Tìm shop alternatives cho 1 item (cùng tên, khác nguồn)
  const getShopAlternatives = (item) => {
    const nameLower = item.name.toLowerCase();
    if (item.shopId) {
      // Đây là shop-only item, tìm nhà hàng + shop khác
      return menuItems.filter(i => i.name.toLowerCase() === nameLower && i.id !== item.id);
    }
    // Đây là món nhà hàng, tìm shop có cùng tên
    return shopItemsAll.filter(i => i.name.toLowerCase() === nameLower);
  };

  return (
    <View style={styles.container}>
      <StatusBar barStyle="light-content" backgroundColor={COLORS.primary} />
      <View style={styles.header}>
        <View>
          <Text style={styles.headerHi}>Xin chào, {currentUser?.name || 'Khách'} 👋</Text>
          <Text style={styles.headerSub}>{restaurantInfo.name}</Text>
        </View>
        <TouchableOpacity
          onPress={() => isGuest ? navigation.navigate('Login') : navigation.navigate('CustomerCart')}
          style={styles.cartBtn}
        >
          <Ionicons name={isGuest ? 'log-in-outline' : 'cart-outline'} size={28} color="#fff" />
          {!isGuest && cartCount > 0 && (
            <View style={styles.badge}><Text style={styles.badgeText}>{cartCount}</Text></View>
          )}
        </TouchableOpacity>
      </View>

      <View style={styles.searchBox}>
        <Ionicons name="search" size={18} color={COLORS.gray} />
        <TextInput style={styles.searchInput} placeholder="Tìm món ăn..." value={search} onChangeText={setSearch} placeholderTextColor={COLORS.gray} />
        {search.length > 0 && <TouchableOpacity onPress={() => setSearch('')}><Ionicons name="close-circle" size={18} color={COLORS.gray} /></TouchableOpacity>}
      </View>

      <View style={styles.catsWrap}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.cats}>
          {CATEGORIES.map(c => (
            <TouchableOpacity key={c} style={[styles.chip, cat === c && styles.chipActive]} onPress={() => setCat(c)}>
              <Text style={[styles.chipText, cat === c && styles.chipTextActive]}>{c}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>

      <FlatList
        data={filtered} keyExtractor={i => i.id} numColumns={2}
        contentContainerStyle={styles.list}
        renderItem={({ item }) => {
          const inWindow = isInSaleWindow(item);
          const canOrder = item.available && inWindow;
          return (
            <TouchableOpacity style={styles.card} onPress={() => { if (isGuest) { navigation.navigate('Login'); return; } canOrder && setModal({ visible: true, item }); }} activeOpacity={0.85}>
              <View style={styles.imgWrap}>
                {item.image
                  ? <Image source={{ uri: item.image }} style={styles.img} />
                  : <View style={[styles.img, styles.imgPlaceholder]}><Ionicons name="fast-food-outline" size={38} color={COLORS.primary} /></View>}
                {!item.available && (
                  <View style={styles.soldOut}><Text style={styles.soldOutText}>Hết hàng</Text></View>
                )}
                {item.available && !inWindow && (
                  <View style={[styles.soldOut, { backgroundColor: 'rgba(30,30,30,0.65)' }]}>
                    <Ionicons name="time-outline" size={16} color="#FFD54F" />
                    <Text style={styles.soldOutText}>
                      Từ {item.saleStart}
                    </Text>
                  </View>
                )}
                {item.shopId && (
                  <View style={styles.shopBadge}>
                    <Ionicons name="storefront" size={10} color="#fff" />
                    <Text style={styles.shopBadgeText} numberOfLines={1}>{item.shopName}</Text>
                  </View>
                )}
                {!item.shopId && getShopAlternatives(item).length > 0 && (
                  <View style={styles.altBadge}>
                    <Text style={styles.altBadgeText}>+{getShopAlternatives(item).length} quán</Text>
                  </View>
                )}
                {item.toppings?.length > 0 && (
                  <View style={styles.tBadge}>
                    <Ionicons name="add-circle" size={11} color="#fff" />
                    <Text style={styles.tBadgeText}>{item.toppings.length} kèm</Text>
                  </View>
                )}
              </View>
              <View style={styles.cardBody}>
                <Text style={styles.cardName} numberOfLines={2}>{item.name}</Text>
                {item.saleStart && (
                  <Text style={styles.cardTime}>
                    <Ionicons name="time-outline" size={10} color="#FF9800" /> {item.saleStart}–{item.saleEnd}
                  </Text>
                )}
                {item.toppings?.length > 0 && (
                  <Text style={styles.cardTopping} numberOfLines={1}>
                    Kèm: {item.toppings.slice(0,2).map(t => t.name.replace('Thêm ','')).join(', ')}
                    {item.toppings.length > 2 ? '...' : ''}
                  </Text>
                )}
                <View style={styles.cardFoot}>
                  <Text style={styles.cardPrice}>{formatCurrency(item.price)}</Text>
                  <View style={[styles.addCircle, !canOrder && { backgroundColor: COLORS.gray }]}>
                    <Ionicons name="add" size={18} color="#fff" />
                  </View>
                </View>
              </View>
            </TouchableOpacity>
          );
        }}
        ListEmptyComponent={
          <View style={styles.empty}>
            <Ionicons name="fast-food-outline" size={60} color={COLORS.lightGray} />
            <Text style={styles.emptyText}>Không tìm thấy món</Text>
          </View>
        }
      />

      <ToppingModal
        visible={modal.visible} item={modal.item}
        shopAlternatives={modal.item ? getShopAlternatives(modal.item) : []}
        onClose={() => setModal({ visible: false, item: null })}
        onConfirm={(item, toppings) => { if (isGuest) { navigation.navigate('Login'); return; } addToCart(currentUser.id, item, toppings); }}
      />
    </View>
  );
}

const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: COLORS.background },
  header: {
    backgroundColor: COLORS.primary, padding: 16, paddingTop: 48,
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
  },
  headerHi: { fontSize: 18, fontWeight: 'bold', color: '#fff' },
  headerSub: { fontSize: 12, color: COLORS.secondary, marginTop: 2 },
  cartBtn: { position: 'relative', padding: 4 },
  badge: {
    position: 'absolute', top: -4, right: -4,
    backgroundColor: COLORS.dark, borderRadius: 10,
    width: 20, height: 20, justifyContent: 'center', alignItems: 'center',
  },
  badgeText: { color: '#fff', fontSize: 11, fontWeight: 'bold' },
  searchBox: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: '#fff', margin: 12, borderRadius: 12,
    paddingHorizontal: 12, elevation: 2,
  },
  searchInput: { flex: 1, height: 42, marginLeft: 8, fontSize: 14, color: COLORS.dark },
  catsWrap: { backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: COLORS.lightGray },
  cats: { paddingHorizontal: 12, paddingVertical: 8, gap: 8 },
  chip: {
    paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20, marginRight: 8,
    backgroundColor: '#fff', borderWidth: 1, borderColor: COLORS.lightGray,
  },
  chipActive: { backgroundColor: COLORS.primary, borderColor: COLORS.primary },
  chipText: { fontSize: 13, color: COLORS.gray },
  chipTextActive: { color: '#fff', fontWeight: 'bold' },
  list: { padding: 8, paddingBottom: 20 },
  card: {
    flex: 1, margin: 6, backgroundColor: '#fff', borderRadius: 14,
    overflow: 'hidden', elevation: 3,
  },
  imgWrap: { position: 'relative' },
  img: { width: '100%', height: 118 },
  imgPlaceholder: { backgroundColor: COLORS.lightGray, justifyContent: 'center', alignItems: 'center' },
  soldOut: {
    position: 'absolute', inset: 0,
    backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center',
  },
  soldOutText: { color: '#fff', fontWeight: 'bold' },
  tBadge: {
    position: 'absolute', top: 6, left: 6,
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: COLORS.primary, borderRadius: 10, paddingHorizontal: 6, paddingVertical: 2,
  },
  tBadgeText: { color: '#fff', fontSize: 10, marginLeft: 2, fontWeight: 'bold' },
  cardBody: { padding: 10 },
  cardName: { fontSize: 13, fontWeight: 'bold', color: COLORS.dark },
  cardTime: { fontSize: 10, color: '#FF9800', marginTop: 2 },
  cardTopping: { fontSize: 11, color: COLORS.gray, marginTop: 2 },
  cardFoot: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 6 },
  cardPrice: { fontSize: 14, fontWeight: 'bold', color: COLORS.primary },
  addCircle: { backgroundColor: COLORS.primary, borderRadius: 18, width: 30, height: 30, justifyContent: 'center', alignItems: 'center' },
  empty: { alignItems: 'center', paddingTop: 80 },
  emptyText: { color: COLORS.gray, fontSize: 15, marginTop: 10 },
  // Modal
  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'center', paddingHorizontal: 20 },
  sheet: { backgroundColor: '#fff', borderRadius: 20, paddingBottom: 20, maxHeight: '80%' },
  sheetHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', paddingTop: 12, paddingHorizontal: 14, paddingBottom: 4 },
  handle: { display: 'none' },
  closeBtn: { width: 32, height: 32, borderRadius: 16, backgroundColor: COLORS.lightGray, justifyContent: 'center', alignItems: 'center' },
  itemRow: { flexDirection: 'row', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: COLORS.lightGray },
  itemIcon: { width: 56, height: 56, borderRadius: 12, backgroundColor: COLORS.lightGray, justifyContent: 'center', alignItems: 'center', marginRight: 12, overflow: 'hidden' },
  itemImg: { width: 56, height: 56 },
  itemName: { fontSize: 16, fontWeight: 'bold', color: COLORS.dark },
  itemPrice: { fontSize: 14, color: COLORS.primary, fontWeight: '600', marginTop: 4 },
  toppingTitle: { fontSize: 14, fontWeight: 'bold', color: COLORS.dark, paddingHorizontal: 16, paddingTop: 12, paddingBottom: 6 },
  tRow: { flexDirection: 'row', alignItems: 'center', marginHorizontal: 16, marginBottom: 6, padding: 10, borderRadius: 10, backgroundColor: COLORS.lightGray, borderWidth: 1.5, borderColor: 'transparent' },
  tRowActive: { backgroundColor: COLORS.primary + '15', borderColor: COLORS.primary },
  check: { width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: COLORS.gray, justifyContent: 'center', alignItems: 'center', marginRight: 10 },
  checkActive: { backgroundColor: COLORS.primary, borderColor: COLORS.primary },
  tName: { flex: 1, fontSize: 13, color: COLORS.dark },
  tPrice: { fontSize: 13, color: COLORS.gray },
  totalRow: { flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 16, marginTop: 10, padding: 10, backgroundColor: COLORS.background, borderRadius: 10 },
  totalLabel: { color: COLORS.gray, fontSize: 14 },
  totalVal: { fontSize: 16, fontWeight: 'bold', color: COLORS.primary },
  modalBtns: { flexDirection: 'row', margin: 16, gap: 10 },
  skipBtn: { flex: 1, padding: 13, borderRadius: 12, alignItems: 'center', borderWidth: 1.5, borderColor: COLORS.lightGray },
  skipText: { color: COLORS.gray, fontWeight: '600' },
  addBtn: { flex: 2, flexDirection: 'row', padding: 13, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.primary, gap: 6 },
  addText: { color: '#fff', fontWeight: 'bold', fontSize: 14 },
  // Source picker
  sourceSection: { paddingHorizontal: 14, paddingTop: 10, paddingBottom: 6, borderBottomWidth: 1, borderBottomColor: COLORS.lightGray },
  sourceLabel: { fontSize: 12, color: COLORS.gray, marginBottom: 8, fontWeight: '600' },
  sourceRow: { gap: 8 },
  sourceChip: { flexDirection: 'column', alignItems: 'center', backgroundColor: COLORS.lightGray, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 8, borderWidth: 1.5, borderColor: 'transparent', gap: 2, minWidth: 80 },
  sourceChipActive: { backgroundColor: COLORS.primary, borderColor: COLORS.primary },
  sourceChipText: { fontSize: 11, color: COLORS.dark, fontWeight: '600', textAlign: 'center' },
  sourceChipTextActive: { color: '#fff' },
  sourceChipPrice: { fontSize: 12, color: COLORS.primary, fontWeight: 'bold' },
  shopSourceTag: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 2, marginBottom: 2 },
  shopSourceText: { fontSize: 11, color: '#2196F3', fontWeight: '600' },
  // Card badges
  shopBadge: { position: 'absolute', bottom: 6, left: 6, flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: '#2196F3', borderRadius: 8, paddingHorizontal: 6, paddingVertical: 2 },
  shopBadgeText: { color: '#fff', fontSize: 9, fontWeight: 'bold', maxWidth: 80 },
  altBadge: { position: 'absolute', bottom: 6, right: 6, backgroundColor: '#FF9800', borderRadius: 8, paddingHorizontal: 6, paddingVertical: 2 },
  altBadgeText: { color: '#fff', fontSize: 9, fontWeight: 'bold' },
});
