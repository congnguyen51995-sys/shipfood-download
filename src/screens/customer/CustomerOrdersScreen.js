...
const FILTERS = ['Tất cả', 'Đang giao', 'Đã giao', 'Đã hủy'];
...
export default function CustomerOrdersScreen({ navigation }) {
  ...
  const orders = filter === 'Tất cả' ? allOrders
    : filter === 'Đang giao' ? allOrders.filter(o => !['Đã giao', 'Đã hủy'].includes(o.status))
...
const styles = StyleSheet.create({
  ...
  filterBar: { backgroundColor: '#fff', maxHeight: 48, borderBottomWidth: 1, borderBottomColor: '#F0F0F0' },
  filterContent: { paddingHorizontal: 12, paddingVertical: 8, gap: 8 },
  filterChip: { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20, backgroundColor: '#F5F5F5' },
